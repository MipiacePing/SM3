...
ROL = lambda X,i:((X << (i&31)) & 0xFFFFFFFF) | ((X & 0xFFFFFFFF) >> (32 - (i&0x31)))

FF = lambda X, Y, Z, j: (X ^ Y ^ Z) if (j >= 0 and j <= 15) else ((X & Y) | (X & Z) | (Y & Z))

GG = lambda X, Y, Z, j: (X ^ Y ^ Z) if (j >= 0 and j <= 15) else ((X & Y) | (~X & Z))

P0 = lambda X:(X ^ ROL(X, 9) ^ ROL(X, 17))

P1 = lambda X:(X ^ ROL(X, 15) ^ ROL(X, 23))

T_ = lambda j: T[0] if (j>=0 and j<=15) else T[1]
...
if __name__ == '__main__':
    print(SM3(b2a_hex('Hello World'.encode())))
